...
namespace i2c {
    ...
    uint8_t init(i2c_port_t Port, i2c_mode_t Mode, uint32_t Frequency, gpio_num_t Sda, gpio_num_t Scl) {
        ...
        i2c_config_t conf = {.mode = Mode, .sda_io_num = Sda, .sda_pullup_en = GPIO_PULLUP_ENABLE, .scl_io_num = Scl, .scl_pullup_en = GPIO_PULLUP_ENABLE};
    ...
    // der Data Pointer sollte auf ein Array zeigen, welches mindestend so gross wie
    // die erwarteten Datenpakete ist
    /**
     * @brief
     *
     * @param SlaveAddress
    ...
    // Rückgabewert: True wenn Acknowledged vom Slave, False wenn die Übertragung
    // scheiterte
    /**
     * @brief
     *

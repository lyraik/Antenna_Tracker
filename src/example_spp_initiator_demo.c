...
#include  "mavlink/c_library_v2-master/c_library_v2-master/common/mavlink.h"
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_DATA_IND_EVT:
        ...
        counter = 0;    
        while(counter <= (param->data_ind.len))
        {
            raw_data = ((param->data_ind.data) + counter);

            if(mavlink_parse_char(chan,*raw_data, &msg, &status))
            {
                if(msg.msgid == 33)
                {

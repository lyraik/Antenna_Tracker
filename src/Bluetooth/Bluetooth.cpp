...
namespace bt {
    ...
    etl::vector<utils::String, DISCOVER_DEVICE_COUNT> devices;
    etl::vector<utils::String, DISCOVER_DEVICE_COUNT> deviceUids;
    utils::String remoteDeviceUid;
    ...
    void esp_bt_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t* param) {
        switch (event) {
            ...
            case ESP_BT_GAP_DISC_RES_EVT: {
                ...
                // add device uid to list of detected devices
                auto end = deviceUids.end();
                auto iter = std::find(deviceUids.begin(), end, utils::StringView{(const char*)param->disc_res.bda, ESP_BD_ADDR_LEN});
                if (iter == end) {
                    deviceUids.emplace_back((const char*)param->disc_res.bda, (size_t)ESP_BD_ADDR_LEN, true);
                    for (const utils::String& e : deviceUids) ESP_LOGI(SPP_TAG, "element '%s'", (const char*)e.str());
                }
                ...
                for (int i = 0; i < param->disc_res.num_prop; i++) {
                    if (param->disc_res.prop[i].type == ESP_BT_GAP_DEV_PROP_EIR
                        ...
                        // add device name to list of detected devices
                        if (iter == end) {
                            devices.emplace_back(peer_bdname, peer_bdname_len, true);
                            break;
                        }

                        // connect to device if uid is empty and name matches
                        if (remoteDeviceUid.empty() && defaultDeviceName == utils::StringView{peer_bdname, peer_bdname_len}) {
                            remoteDeviceUid.copy((const char*)param->disc_res.bda, ESP_BD_ADDR_LEN);
                        }
                    }
                }

                // connect to device if uid matches
                if (remoteDeviceUid == utils::StringView{(const char*)param->disc_res.bda, ESP_BD_ADDR_LEN}) {
    ...
    void connect(const utils::StringView& deviceName) {}

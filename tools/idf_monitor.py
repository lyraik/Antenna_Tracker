...
try:
    ...
    print("Intalling future...")
    os.system("%s -m pip install future"%sys.executable)
from future import standard_library
...
try:
    ...
    print("Intalling builtins...")
    os.system("%s -m pip install builtins"%sys.executable)
from builtins import chr
from builtins import object
from builtins import bytes
import subprocess
...
try:
    ...
    print("Intalling serial...")
    os.system("%s -m pip install pyserial"%sys.executable)
from future import standard_library
import serial
            ...
            while self.alive:
                try:
                    if os.name == 'nt':
                        ...
                        if c == unichr(13):
                            return unichr(10)
                        elif c in (unichr(0), unichr(0x0e)):    # functions keys, ignore
                            continue
                    elif self.test_mode:
...
class Monitor(object):
    ...
    def handle_menu_key(self, c):
        ...
        elif c == CTRL_U:  # upload
            self.run_make(["run", "--target", "upload"])
        elif c == CTRL_B:  # Recompile
            self.run_make("run")
...
""".format(version=__version__,
           ...
           appmake=key_description(CTRL_B),
           makecmd=key_description(CTRL_U),
           output=key_description(CTRL_Y),
           pause=key_description(CTRL_H) )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=conf.monitor_speed)

    parser.add_argument(
        '--make', '-m',
        help='Command to run make',
        type=str, default=conf.pio)

    parser.add_argument(
        '--toolchain-prefix',
        help="Triplet prefix to add before cross-toolchain names",
        default=conf.toolchain)
    ...
    parser.add_argument(
        'elf_file', help='ELF file of application', nargs='?',
        type=argparse.FileType('rb'), default=conf.firmware)
